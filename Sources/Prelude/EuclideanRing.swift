public protocol EuclideanRing: CommutativeRing, Sendable {
...
public func greatestCommonDivisor<A: Equatable & EuclideanRing>(_ x: A, _ y: A) -> A {
    return y == A.zero ? x : greatestCommonDivisor(y, x.mod(y))
}

public func leastCommonMultiple<A: Equatable & EuclideanRing>(_ x: A, _ y: A) -> A {
    return x == A.zero || y == A.zero ? A.zero : x * y / greatestCommonDivisor(x, y)
}
