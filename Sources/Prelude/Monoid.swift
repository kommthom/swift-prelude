...
extension Array: Monoid where Element == Sendable {
  public static let empty: Array = .init()
}

public func joined<M: Monoid>(_ s: M) -> @Sendable ([M]) -> M {
    return { xs in
...
public struct Sum<A: Numeric & Sendable> {
    public let sum: A
...
public struct Product<A: Numeric & Sendable> {
