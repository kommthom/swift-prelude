...
public func bimap<A: Sendable, B: Sendable, C: Sendable, D: Sendable>(_ a2c: @escaping @Sendable (A) -> C) -> @Sendable (@escaping @Sendable (B) -> D) -> @Sendable (Validation<A, B>) -> Validation<C, D> {
    return { b2d in { ab in
            ab.bimap(a2c, b2d)
    } }
}
...
public func apply<A: NearSemiring, B: Sendable, C: Sendable>(_ b2c: Validation<A, @Sendable (B) -> C>) -> @Sendable (Validation<A, B>) -> Validation<A, C> {
    return { ab in
        b2c <*> ab
    }
}
...
public func pure<E, A: Sendable>(_ a: A) -> Validation<E, A> {
    return .valid(a)
}
...
extension Validation: Alt where E: NearSemiring {
    public static func <|> (lhs: Validation, rhs: @autoclosure @escaping @Sendable () -> Validation) -> Validation {
        let rhs = rhs()
        switch (lhs, rhs) {
            case (.invalid, .valid):
                return rhs
